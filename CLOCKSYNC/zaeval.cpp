...
int solve(int switchNum, int nowCount, int sum, vector<int>& clocks) {
	if (switchNum == SWITCH_NUM)
	{
		for (int clock = 0; clock < CLOCK_NUM; ++clock) {
			if (clocks[clock] % 12 > 0) {
				return INF;
			}
		}
		return sum;
	...
	for (int count = 0; count < 4; ++count) {
		answer = min(solve(switchNum + 1, count, sum + nowCount, clocks),answer);
		if (switchNum == SWITCH_NUM - 1)
			break;
	}
